...
build() {
    ...
    if [ ! -d "${CURRENT_DIR}/dist/android" ]; then
        mkdir ${CURRENT_DIR}/dist/android
    fi

    if [ ! -d "${CURRENT_DIR}/dist/android/libevent" ]; then
        mkdir ${CURRENT_DIR}/dist/android/libevent
    fi

    if [ ! -d "${CURRENT_DIR}/dist/android/libevent/${ARCH_PREFIX}" ]; then
        mkdir ${CURRENT_DIR}/dist/android/libevent/${ARCH_PREFIX}
    fi

    PREFIX=${CURRENT_DIR}/dist/android/libevent/${ARCH_PREFIX}

    rm -rf ${PREFIX}

    OPENSSL_LIB_DIR=${CURRENT_DIR}/dist/android/openssl/${ARCH_PREFIX}
    ...
    export PKG_CONFIG_PATH=${OPENSSL_LIB_DIR}/lib/pkgconfig
...
case "$1" in
    ...
    ;;
    clean)
         rm -rf ${CURRENT_DIR}/dist/android/libevent/*
    ;;
esac
