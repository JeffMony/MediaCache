...
build() {
    ...
    if [ ! -d "${CURRENT_DIR}/dist/android/openssl" ]; then
        mkdir ${CURRENT_DIR}/dist/android/openssl
    fi
    if [ ! -d "${CURRENT_DIR}/dist/android/openssl/${ARCH_PLATFORM}" ]; then
        mkdir ${CURRENT_DIR}/dist/android/openssl/${ARCH_PLATFORM}
    fi

    export PATH=${CROSS_PREFIX}:${PATH}
    rm -rf ${CURRENT_DIR}/dist/android/openssl/${ARCH_PLATFORM}
    make clean
    ...
    --openssldir=${CURRENT_DIR}/dist/android/openssl/${ARCH_PLATFORM} \
    --prefix=${CURRENT_DIR}/dist/android/openssl/${ARCH_PLATFORM}
...
case "$1" in
    ...
    ;;
    clean)
        rm -rf ${CURRENT_DIR}/dist/android/openssl/*
    ;;
esac
